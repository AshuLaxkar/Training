...
//        user.setAddress(address);
        String id = generateUniqueId(fakeAvatar);
...
public static String generateUniqueId(ArrayList<FakeAvatar> fakeAvatars){
//        try{
//            int maxId = fakeAvatars.stream()
//                    .mapToInt(fakeAvatar ->Integer.parseInt(fakeAvatar.getId()))
//                    .max()
//                    .orElse(0);
...
//        return String.valueOf(maxId +1);
    int maxNumericId = fakeAvatars.stream()
            .filter(fakeAvatar -> !fakeAvatar.getId().equalsIgnoreCase("id"))
            .map(fakeAvatar -> {
                String id = fakeAvatar.getId();
                try {
    ...
    // Increment the maximum numeric ID to get the new numeric part
    int newNumericId = maxNumericId + 1;

    // Return the new ID with the numeric part as a String
    return String.valueOf(newNumericId);
}
...
//                System.out.print(fakeAvatar.getId());
                bw.write(",");
